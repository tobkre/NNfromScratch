...
def train(X, y, hidden, out, epochs, eta, lmbda, print_loss=False):
...
#        dbhidden = np.reshape(np.sum(bhidden_grad, axis=0)/n_samples, hidden.b.shape)
        ...
        dWout = np.reshape(np.sum(Wout_grad, axis=0)/n_samples, out.W.shape)+ lmbda * out.W
        dbout = np.reshape(np.sum(bout_grad, axis=0)/n_samples, out.b.shape)
#        print('Out')
#        print('dW')
#        print(dWout)
...
#        print(dbout)
        ...
        dWhidden = np.reshape(np.sum(Whidden_grad, axis=0)/n_samples, hidden.W.shape) + lmbda * hidden.W
        dbhidden = np.reshape(np.sum(bhidden_grad, axis=0)/n_samples, hidden.b.shape)
#        print('Hidden')
#        print('dW')
#        print(dWhidden)
...
def test_0():
    ...
    hidden_layer = HiddenLayer(n_input=1, n_nodes=3, X_in=None, dropout_rate=0, activation='sigmoid', seed=2)
    #print(hidden_layer.W)
    output_layer = OutputLayer(n_input=3, n_nodes=1, X_in=None, activation='linear', seed=2)
    
    mse = train(x, y, hidden_layer, output_layer, n_epochs, 0.1, lmbda=0)
    return mse
...
#    print('MSE', mse)
    ...
    hidden_layer = HiddenLayer(n_input=n_features, n_nodes=50, X_in=None, dropout_rate=0, activation='sigmoid', seed=2)
    #print(hidden_layer.W)
    output_layer = OutputLayer(n_input=50, n_nodes=1, X_in=None, activation='linear', seed=2)
    
    my_mse = train(x, y, hidden_layer, output_layer, n_epochs, 0.001, lmbda=0.01)
